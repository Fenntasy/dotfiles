...
- **Always install with exact versions** — use `npm install --save-exact` (or `-E`) so no `^` or `~` ranges are added to `package.json`. Ranges allow silent minor/patch upgrades that can introduce breaking changes or vulnerabilities between installs.
...
- `/react` — React component patterns, hooks, error boundaries, memoization
- `/react-router` — Route type safety, loaders, actions, FormData parsing, useLoaderData, useActionData
