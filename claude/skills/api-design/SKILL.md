...
Language-agnostic REST API design guidance. Covers the _contract_ — what a well-designed API looks like. Implementation skills (TypeScript) handle the _how_.
...
**Permission checks before existence checks** (Google, Microsoft): Return 401/403 _before_ checking if the resource exists. Never leak resource existence to unauthorized callers.
...
- Audit authorization _decisions_ (both grants and denials) — not just successful actions
...
- snake_case for all JSON fields — matches Stripe, GitHub, Zalando, Cloudflare conventions.
