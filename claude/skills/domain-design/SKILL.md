...
> **Scope boundary:** This skill defines *what* to model. For plan structure and task decomposition, see the **Code Planning skill** (`/code-planning`). For implementation patterns, see `/typescript`. For API contract design, see `/api-design`.
...
> **Implementation:** For TypeScript branded types and discriminated unions for state machines, see **TypeScript skill** (`/typescript` §1-2).
...
> **Implementation:** For pagination contracts, see **API Design skill** (`/api-design` §6).
...
> **Implementation:** For project-specific migration steps, check the project's `CLAUDE.md`.
...
- Enforce boundaries with access control (TypeScript: barrel exports with lint rules)
...
| Technology mismatch | ML pipeline needs Python; core app is TypeScript |
