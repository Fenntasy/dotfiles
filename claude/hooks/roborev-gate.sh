...
#
# Allows through ONLY when claude-code has a "done" review for the branch.
# Blocks on: missing reviews, running, queued.
...
# Skip excluded branches (try yq first, fall back to sed)
...
# Query roborev for the resolved branch
...
# Require claude-code to have at least one "done" review
CLAUDE_DONE=$(echo "$REVIEW_JSON" | jq '[.[] | select(.agent == "claude-code" and .status == "done")] | length' 2>/dev/null || echo 0)
if [ "$CLAUDE_DONE" -eq 0 ]; then
  echo "BLOCK: No completed claude-code review for branch '$CURRENT_BRANCH'. Run \`roborev review --branch --agent claude-code\`." >&2
  exit 2
fi
