...
- The user sees only your message text — not tool results, file reads, command output, or thinking — and only reliably the *final* text of a turn: content written before a tool call in the same turn may not render. Anything the user must act on goes in the turn's final message, or inside the interaction itself (e.g. the question text of a dialog). Writing "as shown above" or "see the output" about a tool result is the tell: stop and paste the content instead.
